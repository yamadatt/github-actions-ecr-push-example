...
resource "aws_ecs_service" "mars_g_a" {
  name            = "mars-g-a-service"
  cluster         = aws_ecs_cluster.main.id
  task_definition = aws_ecs_task_definition.mars_g_a.arn
  launch_type     = "FARGATE"
  desired_count   = 1
  network_configuration {
    subnets = [
  ...
  load_balancer {
    ...
    container_name   = local.container_name2
    container_port   = 80
  }
}
